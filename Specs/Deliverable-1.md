...
Your solution must be a client-server solution consisting of multiple projects as outlined in the course. The Presentation Layer portion is to be an ASP.NET Web Application (**.NET Core 9.0** Blazor Components). **Remember to turn off the HTTPS check box.** The BLL, Entity classes and DAL portions of the system are to be placed in a separate class library project (**.NET Core 9.0**).

## The StarTED Database

The database supplied for this lab is an SQL Server database named "StarTED". The following is a **sample** of the connection string that may be used for the Presentation Layer, where xxxx represents your sql server name.

```json
  "StarTEDDb": "Server=xxxx;Database=StarTED;Trusted_Connection=true;MultipleActiveResultSets=true"
```
